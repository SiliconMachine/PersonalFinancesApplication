...
<template>
  <div class="max-w-screen-md mx-auto mt-5 overflow-hidden bg-white shadow sm:rounded-lg">
      ...
      <dl class="sm:divide-y sm:divide-gray-200">
          ...
          >
            <div class="flex-shrink-0">
              <input
                ...
                id="receipt"
                ref="receipt"
                v-on:change="handleReceiptUpload()"
              />
              <a
                v-on:click="selectReceipt()"
        ...
        <div class="py-4 sm:py-5 sm:grid sm:grid-cols-4 sm:gap-4 sm:px-6">
          <button
            ...
            class="px-4 py-2 text-base font-medium text-white bg-blue-600 border border-transparent rounded-md shadow-sm hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
          >
            Save Changes
          </button>
        </div>
...
export default {
  ...
  methods: {
    ...
    saveChanges() {
      ...
      TransactionsAPI.update(this.transaction.id, formData).then(function (
        response
      ) {
