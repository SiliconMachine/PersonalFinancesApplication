  ...
  <div>
    <div class="grid grid-cols-12 gap-4">
      ...
      <div class="col-span-2 mt-5">
        <div class="flex flex-col items-center justify-center mt-1 overflow-hidden">
          <button
            type="button"
            v-on:click="addTransactionRule()"
        ...
        <div class="flex flex-col items-center justify-center mt-1 overflow-hidden">
          <div class="flex items-center justify-center w-full mb-1">
            <input
              type="checkbox"
              value="yes"
              v-model="accountBalanceSync"
              class="mr-1"
            /> Sync account balance
          </div>
          <button
            type="button"
            v-on:click="importTransactions()"
            class="inline-flex items-center px-4 py-2 text-sm font-medium text-white bg-blue-600 border border-transparent rounded-md shadow-sm hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
          >
            Import
          </button>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                  <thead class="bg-gray-50">
                    <tr>
                      ...
                      <th
                        scope="col"
                        class="max-w-xs px-3 py-3 text-xs font-medium tracking-wider text-left text-gray-500 uppercase"
                      >
                        Category
                      </th>
                      <th
...
export default {
  ...
  methods: {
    ...
    importTransactions() {
      TransactionsAPI.import(this.account, this.pendingTransactions).then(
        function (response) {
    ...
    loadPotentialTransactions() {
      ...
      TransactionsAPI.index(params).then(
        function (response) {
        ...
        for (let k = 0; k < existingTransactions.length; k++) {
          if (
            this.validateUnique(
              ...
              existingTransactions[k]
            )
          ) {
            this.pendingTransactions[i].potential_duplicate =
              existingTransactions[k];
            break;
          }
        }
