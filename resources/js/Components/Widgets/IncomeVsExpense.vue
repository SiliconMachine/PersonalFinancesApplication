<template>
  <div class="col-span-8 overflow-hidden bg-white divide-y divide-gray-200 rounded-lg shadow">
  ...
  methods: {
    configureWidget() {
      ...
      TransactionsAPI.index({
        ...
        end_date: moment(this.endDate, "MMM YYYY")
          .endOf("month")
          .format("YYYY-MM-DD"),
      }).then(
        function (response) {
    ...
    primeChartData() {
      this.chartData = [
        {
          label: moment(this.endDate, "MMM, YYYY")
            .subtract("2", "month")
            .format("MMM YYYY"),
          expenses: 0.0,
          income: 0.0,
